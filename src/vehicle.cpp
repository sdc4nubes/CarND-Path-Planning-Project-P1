...
int VehiclePlanner::lanePlanner(double s, double d, vector<vector<double>> sensor_fusion) {
  ...
  if (distance > safe_distance && lane != 2) {
    ...
    vector <double> vehicle = closestVehicle(s, new_lane, sensor_fusion, true);
		target_vehicle_speed = speed_limit;
    if( vehicle[0] < safe_distance) target_vehicle_speed = vehicle[1] * .98;
		if (vehicle[0] < safe_distance * .5) target_vehicle_speed *= .5;
  }
...
int VehiclePlanner::laneCost(double s, int lane, vector<vector<double>> sensor_fusion) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (i != lane && front_vehicle[0] < safe_distance) costs[i] = 15; 
		...
		if (lane != i) {
			if ((back_vehicle[0] < safe_distance * 1.5) ||
				(back_vehicle[0] < safe_distance * 3 && back_vehicle[1] > check_speed))
				costs[i] = 15;
		}
  }
