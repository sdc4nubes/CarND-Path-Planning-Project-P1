  ...
  for(int vehicle = 0; vehicle < sensor_fusion.size(); vehicle++) {
    ...
    if (vehicle_lane == lane) {
      if (direction == true) {
        ...
        if (vehicle_s > s -5 and (vehicle_s - s - 5) < dist) {
          dist = vehicle_s - s;
          velocity = vehicle_v;
        }
      } else {
				// Capture distance and speed of vehicle directly behind
        if (s + 5 >= vehicle_s and (s + 5 - vehicle_s) < dist) {
          dist = s - vehicle_s;
          velocity = vehicle_v;
        }
      }
...
int VehiclePlanner::laneCost(double s, int lane, vector<vector<double>> sensor_fusion) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (lane != i && front_vehicle[0] < unsafe_distance) costs[i] = 15; 
		...
		if (lane != i) {
			if (back_vehicle[0] < unsafe_distance * 2) costs[i] = 15;
			if (front_vehicle[0] > unsafe_distance && front_vehicle[0] < unsafe_distance * 25) {
